...
const TrendGraphs = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`http://localhost:8000/history/processed_data?days=${days}&user_id=1`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      const processedData = await response.json()
      setData(processedData.map(d => ({
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      ...
      <select value={timeRange} onChange={(e) => setTimeRange(e.target.value)} className="mb-4 p-2 border rounded">
