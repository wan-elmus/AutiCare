...
export default function LandingPageClient({ userProfile }) {
  ...
  useEffect(() => {
    console.log('LandingPageClient: UserContext user:', user)
    if (!user && !loading) {
      console.log('LandingPageClient: No user, redirecting to /auth/login')
  ...
  useEffect(() => {
    async function fetchSensorData() {
      if (!user?.id) return
      try {
        const response = await fetch(`${API_URL}/sensor/data?email=${encodeURIComponent(user.email)}`, {
          method: 'POST',
        ...
        const validCoords = dataArray
          .filter((d) => d && d.longitude && d.latitude)
